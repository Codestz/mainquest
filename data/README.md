...
`merges` needs one `search(type:ISSUE ... is:merged)` call **per user**. Search is rate-limited separately at 30 requests/minute, so a few
...
Replace with the curated 192 from `npm run sheet`.

## sprites/

Base64-inlined PNGs, one per class, generated by `scripts/pixelate.py`. See
`ATTRIBUTION.md` for licensing. Remember `image-rendering="pixelated"`.
